...
/*   Updated: 2024/10/03 22:11:00 by thestutteri      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void handle_arg_3(t_exec *data, char *str, int flag, int j)
{
    if (flag == 1)
    {
        while (str[j] != '=')
...
void export_hard(t_exec *data, t_cmd *input, int read_fd, int write_fd)
{
    ...
    if (input->arguments[0])
    {
        ...
        {
            handle_arg(data, input->arguments[i], &flag, input);
            printf("%d\n", flag);
            if (flag == 1)
